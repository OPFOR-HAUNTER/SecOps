<#
.SYNOPSIS
    Gets JSON objects from the SOC Trello board. 
...
function Get-Trello {
    ...
    begin {
        # set API Call values
        $userAgent = 'CARB SOC Trello Board Powershell Script'
        $trelloKey = Get-Content -path "$env:ScriptBase\Tokens\CARB_Trello-key.txt"
        $trelloToken =  Get-Content -path "$env:ScriptBase\Tokens\CARB_Trello-token.txt"
...
.DESCRIPTION
    Creates a new Trello card based on parameter values to the SOC Trello board
.INPUTS
...
.EXAMPLE
    ...
    Get cards from a specific list (default is the Incidents list), other lists ids in carb_lists.txt
        Get-Trello -list -listId 5b69b61773e7c85a6c14b890
